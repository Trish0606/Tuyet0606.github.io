...
function animate(time) {
    ...
    bfs.forEach((bf, i) => {
        ...
        // NATURAL FLIGHT MATH (Zig-zag flutter)
        const flutterX = Math.sin(time * 0.002 + i) * 45 + Math.sin(time * 0.0005 + i) * 130;
        const flutterY = Math.cos(time * 0.002 + i) * 45 + Math.cos(time * 0.0003 + i) * 100;
        ...
        let targetX = (distToMouse < 280) ? mouse.x : homeX + flutterX;
        let targetY = (distToMouse < 280) ? mouse.y : homeY + flutterY;
        
        let speed = (distToMouse < 280) ? 0.08 : 0.022;
        ...
        if (Math.random() > 0.35) particles.push(new Particle(nextX, nextY));
    });
